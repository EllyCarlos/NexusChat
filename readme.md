...
CI validates both Prisma schemas, generates both clients independently, and runs the frontend parity and migration-ownership checks. It does not currently run `prisma migrate deploy` against a disposable database. The existing initial migration assumes the PostgreSQL `extensions` schema already exists before creating `uuid-ossp` there, so clean-database deployment remains uncertified until that prerequisite is exercised with disposable-database evidence. The migration directory currently has no `migration_lock.toml`; Prisma schema validation does not require one.
