...
export type SessionTokenBoundary = "api" | "socket";
...
): SessionTokenPayload => verifySessionTokenForAudience(
  token,
  boundary === "api" ? TOKEN_AUDIENCES.API : TOKEN_AUDIENCES.SOCKET,
);

export const verifyApiSessionToken = (token: string): SessionTokenPayload =>
  verifySessionToken(token, "api");

export const verifySocketSessionToken = (token: string): SessionTokenPayload =>
  verifySessionToken(token, "socket");

export type SessionTokenErrorKind = "expired" | "invalid";

export const classifySessionTokenError = (error: unknown): SessionTokenErrorKind | undefined => {
